...
def koostaAlgtabel(ainult_trajektoorid,seisundite_Sonastik):
...
def klasterdaUus(ainult_trajektoorid,klastriteArv,cdistPath,piir,tee,klNR):
    skoorid = []
    mudelid = []
    sklearnX = loadtxt(cdistPath, delimiter=',')
    formatted_dataset = ainult_trajektoorid[:len(sklearnX)]
    range_n_clusters = list(range(klNR,klastriteArv))
...
# Etteantud n klastrite arvu korral arvutab ainult siluetikoefitsendi väärtuse
...
def leiaSarnaseimadKlastris(best,formatted_dataset,y_pred,kaal,KLnr):  
...
# Funktsioon leidmaks trajektooride mediaan pikkus, klastris esinevate trajektooride pikkuste alusel
...
# Töövoog n erinevate
lopp = {}
for i in range(valPikkus,klastriteArv+1):
    skoorid,mudelid,formatted_dataset = klasterdaUus(ainult_trajektoorid,i+1,maatriksiTee,piir,tee,i)
    best,y_pred = leiaParimMudel(skoorid,mudelid,valPikkus)
    klastrid,unikaalsed =leiaSarnaseimadKlastris(best,formatted_dataset,y_pred,osakaal,i)
    ...
    #looGraafikud(skoorid,best,formatted_dataset,tee,ravitrajektoorid,y_pred)
    tulem, algtabelitrajarv, arv, m, oigedKlastrid = leiaAlgtabelist(tee,tulemusTrajid)
    lopp[i] = [sorted(tulem.values()), algtabelitrajarv, arv, m, oigedKlastrid]

# Valideerimine ja analüüs
for key, value in lopp.items():
    tulemusKlastriteArv=key
    m=value[3]
    oigedKlastrid=value[4]
    arv=value[2]
    tulem=value[0]
    algtabelitrajarv=value[1]
    print()
    print('KLASTRITE ARV:', tulemusKlastriteArv)
    print('Leitud müra klastrid:',m)
    print('Õigesti leitud klastrite arv:',oigedKlastrid)
    print('Algtabelis olevad trajektooride arv:',arv)
    print('Täpsus1:', (tulemusKlastriteArv-m)/tulemusKlastriteArv*100)
    print('Täpsus2:', arv*100/algtabelitrajarv)
    #print('tulemused',tulem,'algtabeli pikks:',algtabelitrajarv,'hinnang:',len(tulem)*100/algtabelitrajarv)
